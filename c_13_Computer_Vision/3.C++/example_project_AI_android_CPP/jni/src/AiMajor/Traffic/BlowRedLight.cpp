        ...
        && (stSetting.ptRegionsSet.size() == stSetting.strLabelsSet.size()))
    {
        for (int i = 0; i < stSetting.strLabelsSet.size(); i++)
        {
            ...
            else if (stSetting.strLabelsSet[i] == "2")
            {
                ...
                m_ptBeginLineB = cv::Point2f(float(m_ptBeginLine[0].x) / float(m_iWidthFrame), float(m_ptBeginLine[0].y) / float(m_iHeightFrame));
                m_ptBeginLineE = cv::Point2f(float(m_ptBeginLine[1].x) / float(m_iWidthFrame), float(m_ptBeginLine[1].y) / float(m_iHeightFrame));
            }
            else if (stSetting.strLabelsSet[i] == "3")
            {
                m_ptEndLine = stSetting.ptRegionsSet[i];
                m_ptEndLineB = cv::Point2f(float(m_ptEndLine[0].x) / float(m_iWidthFrame), float(m_ptEndLine[0].y) / float(m_iHeightFrame));
                m_ptEndLineE = cv::Point2f(float(m_ptEndLine[1].x) / float(m_iWidthFrame), float(m_ptEndLine[1].y) / float(m_iHeightFrame));
            }
            else if (stSetting.strLabelsSet[i] == "4")
            {
                m_ptLeftLine = stSetting.ptRegionsSet[i];
                m_ptLeftLineB = cv::Point2f(float(m_ptLeftLine[0].x) / float(m_iWidthFrame), float(m_ptLeftLine[0].y) / float(m_iHeightFrame));
                m_ptLeftLineE = cv::Point2f(float(m_ptLeftLine[1].x) / float(m_iWidthFrame), float(m_ptLeftLine[1].y) / float(m_iHeightFrame));
            }
            else if (stSetting.strLabelsSet[i] == "5")
            {
                m_ptRightLine = stSetting.ptRegionsSet[i];
                m_ptRightLineB = cv::Point2f(float(m_ptRightLine[0].x) / float(m_iWidthFrame), float(m_ptRightLine[0].y) / float(m_iHeightFrame));
                m_ptRightLineE = cv::Point2f(float(m_ptRightLine[1].x) / float(m_iWidthFrame), float(m_ptRightLine[1].y) / float(m_iHeightFrame));
            }
...
int BlowRedLight::BRL_Exe(cv::Mat& mImage, std::vector<stOutputBlowRedLight_t>& stBlowRedLights)
{
    ...
    else
    {
        ...
        // get state standard light
        switch (BRL_emGetLightState(mImage, m_ptStandardLight))
        {
        ...
        // get state left light
        switch (BRL_emGetLightState(mImage, m_ptLeftLight))
        {
            case emLightState_t::Red:
            {
                m_emLeftSignal = emLightState_t::Red;
                break;
            }
            case emLightState_t::Yellow:
            {
                m_emLeftSignal = emLightState_t::Yellow;
                break;
            }
            case emLightState_t::Green:
            {
                m_emLeftSignal = emLightState_t::Green;
                break;
            }
            default:
                break;
        ...
        for (auto& stTrack : stTracks)
        {
            if (stTrack.isOutOfFrame == false)
            {
                ...
                if (p != m_stTracks.end()) // if find track in list tracked
                {
                    ...
                    if ((m_emStandardSignal == emLightState_t::Red) || (m_emLeftSignal == emLightState_t::Red))
                    {
                    ...
                    if (m_stTracks[iIndex].bStraight == true)
                    {
                    ...
                    if (m_stTracks[iIndex].bLeft == true)
                    {
                        if (m_pTracker->checkIntersection(stTrack, static_cast<float>(m_iWidthFrame), static_cast<float>(m_iHeightFrame), 
                    ...
                    if (m_stTracks[iIndex].bRight == true)
                    {
                        if (m_pTracker->checkIntersection(stTrack, static_cast<float>(m_iWidthFrame), static_cast<float>(m_iHeightFrame), 
                ...
                else // if find new track
                {
                                                        ...
                                                        RoadLine(m_ptBeginLineB, m_ptBeginLineE, 0)))
                    {
                        ...
                        {
                            stOutputBRL.bStraight = true;
                        }

                        if ((m_emLeftSignal == emLightState_t::Red) 
                            || ((BRL_bCheckAllowedTurn(m_iAllowedLeft, stTrack.m_type) == false) && (m_emStandardSignal == emLightState_t::Red)))
                        {
                            stOutputBRL.bLeft = true;
                        }

                        if ((m_emLeftSignal == emLightState_t::Red) 
                            || ((BRL_bCheckAllowedTurn(m_iAllowedRight, stTrack.m_type) == false) && (m_emStandardSignal == emLightState_t::Red)))
                        {
                            stOutputBRL.bRight = true;
                        }
...
Modified: 
    ...
    <Change>
-------------------------------------------------------------------------------*/
emLightState_t BlowRedLight::BRL_emGetLightState(cv::Mat& mImage, std::vector<std::vector<cv::Point>> ptLights)
{
    if (ptLights.size() == 0)
    {
...
Modified: 
    ...
    <Change>
-------------------------------------------------------------------------------*/
bool BlowRedLight::BRL_bCheckAllowedTurn(int iAllowedTurn, std::string strType)
{
    switch (iAllowedTurn)
    {
        case 0:
