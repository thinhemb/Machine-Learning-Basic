...
int NcnnDetect::runNcnnDetect(const cv::Mat& img, std::vector<std::string> &labels, std::vector<Object> &objects, float threshold , int frame)
{
    int img_w = img.cols;
    int img_h = img.rows;
    ...
    for(int i=0; i<out.h; i++) {
        ...
        try {
            if(values[1] >= threshold) {
                ...
                object.rect.x = (values[2] < 0) ? 0 : (values[2] * img_w);
                object.rect.y = (values[3] < 0) ? 0 : (values[3] * img_h);
