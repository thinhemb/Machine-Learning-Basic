...
void PedestrianTracker::SolveAssignmentProblem(
    ...
    //Mục đích của mã này là tính toán sự khác biệt giữa các detections đang hoạt động và các bản phát hiện để có thể so sánh chúng dễ dàng hơn.
    ComputeDissimilarityMatrix(track_ids, detections, descriptors,
                               &dissimilarity);
    //Thuật toán KuhnMunkres được sử dụng để giải ma trận không tương đồng.
...
{  
    ...
    // Nó lấy danh sách TrackedObjects (phát hiện) 
    // và lọc chúng dựa trên độ tin cậy, tỷ lệ khung hình và chiều cao của chúng.
    ...
    //  Compute method from descriptor_fast and stores it in the descriptors vector.
    // Tính toán phương thức từ descriptor_fast và lưu trữ nó trong vectơ mô tả.
    ...
    if (!active_tracks.empty() && !detections.empty()) 
    {
        ...
        std::set<std::tuple<size_t, size_t, float>> matches;
        //nó sẽ tìm ra các khoânh khố không tương đối và các khoânh khố tương đối 
        // và lưu chúng vào hai tệp hợp unmatched_tracks và unmatched_detections.
        ...
        if (distance_strong_) {
            // Sau đó, nó lặp qua tập hợp các bộ dữ liệu và kiểm tra xem độ tin cậy có ở dưới 
            // một ngưỡng nhất định (params_.aff_thr_fast) 
            // và trên một ngưỡng khác (params_.strong_affinity_thr) hay không.
            std::vector<std::pair<size_t, size_t>> reid_track_and_det_ids =
                GetTrackToDetectionIds(matches);
            //Nó lấy một khung, các phát hiện và một vectơ gồm các cặp ID theo dõi và phát hiện làm tham số.
        ...
        for (const auto &match : matches) 
        {
            ...
            std::string name_image="/home/thinhdv/WorkSpace/pedestrian_tracker_demo/cpp/Data/image_des/"+std::to_string(track_id)+"_"+std::to_string(max)+".jpg";
            // cv::imwrite(name_image, is_matching_to_track[track_id].second);
            max++;
...
PedestrianTracker::StrongMatching(
    ...
    /*  Nó lấy một khung, các phát hiện và một vectơ gồm các cặp ID theo dõi và phát hiện làm tham số. 
    Sau đó, nó tạo một bản đồ lưu trữ ID theo dõi và việc phát hiện có khớp hay không, cũng như bộ mô tả cho mỗi lần phát hiện. Sau đó, 
    nó tính toán khoảng cách giữa khung hình, phát hiện và ID theo dõi/phát hiện. 
    Đối với mỗi cặp ID theo dõi và ID phát hiện trong vectơ, nó sẽ tính toán mối quan hệ giữa đối tượng được phát hiện cuối cùng từ theo dõi và 
    phát hiện hiện tại. Cuối cùng, nó xác định xem mối quan hệ nhận dạng lại có vượt quá một ngưỡng nhất định hay không và nếu có, 
    hãy thêm phần này vào bản đồ xem nó có khớp hay không cũng như bộ mô tả của nó.*/    
